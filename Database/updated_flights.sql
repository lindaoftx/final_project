...
alter table air_travel_stage 
ADD column is_delayed INT
    
    
 -- Add column to display binary numbers for machine learning   
    ALTER TABLE tbl_name ALTER COLUMN col_name TYPE varchar (11);
    
select * from public.air_travel_stage
    where is_delayed = 0
update air_travel_stage
        set is_delayed =   CASE  
                                    WHEN delay_carrier_in_minutes > 0 
                                    OR delay_weather_in_minutes > 0
                                    OR delay_national_aviation_system_in_minutes > 0
                                    OR delay_security_in_minutes > 0
                                    OR delay_late_aircraft_arrival_in_minutes > 0
                                    THEN 1 ELSE 0
                            END
    
alter table flights
ADD column is_delayed SMALLINT
    
update flights
        set is_delayed =   CASE  
                                    WHEN delay_carrier > 0 
                                    OR delay_weather > 0
                                    OR delay_national_aviation_system > 0
                                    OR delay_security > 0
                                    OR delay_late_aircraft_arrival > 0
                                    THEN 1 ELSE 0
                            END
